...
export const Photo = () => {
    ...
    const getMargin = (i) => {
        ...
        try{
            console.log(marginPrev)
            const img = new Image();
            img.src = `http://localhost:7653/images/${login}/${userPhotoes[i-5].filename}`;
            let del = img.width / 150
            if((img.height / del) < 200){
            ...
            <Box sx={style}>
                <div className={'showed'}>
                    ...
                    <div className={'imageIntaractive'}>
                        ...
                        <button onClick={() => {
                            filename.like?.includes(user.login)? PhotoAPI.unlike({'id': filename.id, 'login': user.login, 'likeTo': login}).then(() => {
                            authAPI.getUserPhotoes(login).then(e => {
                                setUserPhotoes(e.data); 
                                e.data.forEach(el => { 
                                    if(el.id === filename.id) setFilename(el)
                            ...
                            }) : PhotoAPI.like({'login': user.login, 'filename': filename.filename, 'likeTo': login}).then(() => {
                                authAPI.getUserPhotoes(login).then(e => {
                                    setUserPhotoes(e.data); 
                                    e.data.forEach(el => { 
                                        if(el.id === filename.id) setFilename(el)
